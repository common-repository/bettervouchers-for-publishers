...
register_activation_hook(   __FILE__, 'BetterVouchersForPublishers_on_activation' );
...
add_action( 'save_post', 'update_bv_option' );

function BetterVouchersForPublishers_on_activation() {
      	...
      	foreach ( $posts as $post ) {
    		...
    		$debug_link = '<a href="'.get_permalink( $post->ID ).'"">'.get_the_title( $post->ID ).'</a>'; 
     		...
     		foreach($marray as $mid => $murl) {
     			if(isset($murl) && (strlen($murl) > 0) && strpos(rawurldecode($post->post_content), $murl) > 0) {
     				$match++; $tot_match++;
     				$matched_url .= " ".$murl.',';
     				$mids .= $mid.',';
     			}
     			...
     			foreach($mkeywords as $mid => $mkeyword) {
	 				...
	 				if(strlen($mkeyword ) > 0) {
		 				...
		 				foreach($tkeywords as $tkey) {
				 			...
				 			if(isset($tkey) && (strlen($tkey) > 3) && strpos(strtolower($post->post_content), strtolower($tkey)) > 0) {
...
function BetterVouchersForPublishers_install() {
	...
	BetterVouchersForPublishers_on_activation();
}
...
function update_bv_option($post_id){
	$current_execution_time = ini_get('max_execution_time'); 
    ini_set('max_execution_time', 600);
